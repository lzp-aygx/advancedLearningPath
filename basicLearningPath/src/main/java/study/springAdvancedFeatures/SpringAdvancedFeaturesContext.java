 ...
 *
 * @Enable*注解的介绍和工作原理 1.@EnableAspectJAutoProxy: 开启对AspectJ自动代理的支持.
...
@PropertySource("classpath:/app.properties")//引用静态配置文件
public class SpringAdvancedFeaturesContext implements AsyncConfigurer {
    ...
    @Bean
    public RunTimeInject inject() {
        return new RunTimeInject(env.getProperty("project.name", "springStudy"),
                env.getProperty("project.author", "lizhupeng"),
                env.getProperty("project.versions",double.class,1.0)
        );
    }
